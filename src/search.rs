...
pub fn knn_exact_search(query: &VecData, k_for_search: usize, dataset: &Dataset) -> SearchResult {
    let mut index = index::brute_force::Index();
    index.knn(dataset, query, k_for_search)
}

pub fn knn_search(
    using_index: IndexType,
    query: Rc<VecData>,
    k_for_search: usize,
    dataset: &Dataset,
) -> SearchResult {
    ...
    match using_index {
        IndexType::BruteForce => {
            ...
            ground_truth = index.knn_rc(dataset, query, k_for_search);
        ...
        IndexType::IVFFlat => {
            ...
            println!(
                "[Info] build index: k for kmeans={}, max loop={}",
                cluster_num, kmeans_max_loop
            );
            let timer = time::Instant::now();
        ...
        IndexType::HNSW => {
            ...
            let timer = time::Instant::now();
            let mut index =
                index::hnsw::Index::build(dataset, pg_max_degree, num_layers, search_queue_size);
            println!("[Info] -> completed: {:?}", timer.elapsed());
