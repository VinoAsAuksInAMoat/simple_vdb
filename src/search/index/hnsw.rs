...
use crate::search::{
    distance, distance::interface::*, distance::l2distance::L2Distance, index::interface::*,
};
...
impl Layer {
    ...
    fn search_layer(
        ...
        let mut candidates: BinaryHeap<Reverse<Neighbor>> =
            BinaryHeap::from([Reverse(start_point.clone())]);
    ...
    pub fn build_naive_pg(
        dataset: &Dataset,
        dataid_set: Vec<DataId>,
        pg_max_degree: Degree,
    ) -> Self {
        let mut layer = Layer {
        ...
        for dataid in dataid_set.iter() {
            ...
            let neighbors =
                layer.find_knn_naive(dataset, Rc::clone(vecdata), pg_max_degree as usize);
...
impl Index {
    pub fn build(
        ...
        layers.insert(
            0,
            Layer::build_naive_pg(dataset, dataid_set.clone(), pg_max_degree),
        );
        if num_layers == 1 {
            return Self {
                layers: layers,
...
impl AnnSearch for Index {
    ...
    fn knn_rc(&mut self, dataset: &Dataset, query: Rc<VecData>, k: usize) -> SearchResult {
        ...
        for layerid in self.layers.len() - 1..1 {
            let neighbors = self.layers[&(layerid as u8)].search_layer(
                dataset,
                Rc::clone(&query),
                start_point,
                1,
        ...
        let neighbors = self.layers[&0].search_layer(
            dataset,
            Rc::clone(&query),
            start_point,
            self.search_queue_size,
        );
